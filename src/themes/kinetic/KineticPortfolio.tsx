...
export default function KineticPortfolio({ settings, repos, reposLoading }: Props) {
  ...
  return <main className="kinetic-theme">
    ...
    {!!settings.certifications?.length && <section className="kinetic-section kinetic-history kinetic-certifications"><span className="kinetic-index">06 / Certifications</span>{settings.certifications.map((item,i) => <motion.article key={i} {...reveal}><span>{item.issue_date}{item.expiry_date ? ` — ${item.expiry_date}` : ''}</span><div><h3>{item.name}</h3><p>{item.issuer}</p>{item.credential_id && <p>Credential ID: {item.credential_id}</p>}{item.credential_url && <a href={item.credential_url} target="_blank" rel="noreferrer">Verify credential ↗</a>}</div></motion.article>)}</section>}
    {!!settings.services?.length && <section className="kinetic-section kinetic-editorial kinetic-services"><span className="kinetic-index">07 / Services</span><h2>Ways we can work together.</h2><div className="kinetic-editorial-grid">{settings.services.map((item,i) => <motion.article key={i} {...reveal}><span>0{i+1}</span><h3>{item.title}</h3><p>{item.description}</p>{item.price && <small>{item.price}</small>}{item.link && <a href={item.link} target={item.link.startsWith('http') ? '_blank' : undefined} rel="noreferrer">{item.link_label || 'Get in touch'} ↗</a>}</motion.article>)}</div></section>}
    {!!settings.testimonials?.length && <section className="kinetic-section kinetic-recognition kinetic-testimonials"><span className="kinetic-index">08 / Testimonials</span><div>{settings.testimonials.map((item,i) => <motion.article key={i} {...reveal}><b>“</b><span>{item.quote}</span><small>{item.name}{item.role || item.company ? ` — ${[item.role,item.company].filter(Boolean).join(', ')}` : ''}</small></motion.article>)}</div></section>}
