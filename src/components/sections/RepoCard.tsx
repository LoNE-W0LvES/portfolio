...
export default function RepoCard({ repo, langColors }: Props) {
  ...
  const toggleReadme = async (e: React.MouseEvent) => {
    e.preventDefault()
  ...
  const renderReadme = (text: string) => {
    // Very light markdown → HTML (headings, bold, italic, code, links)
    let html = text
    ...
    <div className={`repo-card ${expanded ? 'repo-card-expanded' : ''}`}>
      <div className="repo-card-main">
        <div className="repo-card-header">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="16" height="16" className="repo-icon"><path d="M3 7V5a2 2 0 012-2h2M17 3h2a2 2 0 012 2v2M21 17v2a2 2 0 01-2 2h-2M7 21H5a2 2 0 01-2-2v-2"/><rect x="7" y="7" width="10" height="10" rx="1"/></svg>
        ...
        <div className="repo-meta">
          ...
          {repo.stargazers_count > 0 && (
            <span className="repo-stat">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
              {repo.stargazers_count}
            </span>
          )}
          {repo.forks_count > 0 && (
            <span className="repo-stat">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13"><line x1="6" y1="3" x2="6" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 01-9 9"/></svg>
              {repo.forks_count}
            </span>
          )}
        </div>
        ...
        <div className="repo-actions">
          <button className="repo-readme-btn" onClick={toggleReadme}>
            {expanded ? (
              <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14"><polyline points="18 15 12 9 6 15"/></svg> Hide README</>
            ) : (
              <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14"><polyline points="6 9 12 15 18 9"/></svg> View README</>
            )}
          </button>
          ...
          {repo.homepage && (
            <a href={repo.homepage} target="_blank" rel="noreferrer" className="repo-open-btn repo-open-btn-accent">
              Live Demo
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>
            </a>
          )}
        </div>
      </div>
